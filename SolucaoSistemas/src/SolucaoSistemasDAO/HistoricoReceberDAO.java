 ...
 * @author Batman
 */
public class HistoricoReceberDAO {
    ...
    public boolean CadastrarHistoricoReceberDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "insert into historico_receber (datacadastro, parcela, formapgto, "
                    + "valorpago, quitar) values(now(),?,(select codigo from planocontas where tipo in('3. RECEITA') "
                    + "and descricao = '"+getFormaPgto()+"'),?,?)";
            ...
            System.out.println("Erro ao CadastrarHistoricoReceberDAO "+e.getMessage());
            return false;
        }
    }
}
