 ...
 * @author Batman
 */
public class ItensCompraCanceladaDAO {
    ...
    public boolean CadastrarItensCompraCanceladaDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "insert into itenscompra_cancelada "
                    + "(produto, quantidade, unitario, subtotal, compra) "
                    + "values(?,?,?,?,?)";
            ...
            pst.setInt(5, Integer.parseInt(getCompra()));
            pst.execute();
            pst.close();
            ...
            System.out.println("Erro ao CadastrarItensCompraCanceladaDAO "+e.getMessage());
            return false;
        }
