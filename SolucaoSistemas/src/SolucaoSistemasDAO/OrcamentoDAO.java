...
public class OrcamentoDAO {
    ...
    public boolean CadastrarOrcamentoDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "insert into orcamento (cliente, animal, desconto, total, "
            ...
            System.out.println("Erro ao CadastrarOrcamentoDAO "+e.getMessage());
            return false;
        }
    }
    
    public boolean CancelarOrcamentoDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "update orcamento set situacao = ? where codigo = ?";
            pst = ConectaBanco.con.prepareStatement(comando);
            ...
            System.out.println("Erro ao CancelarOrcamentoDAO "+e.getMessage());
            return false;
        }
    }
    public ResultSet ConsultarOrcamentoDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "select o.codigo, p.nome, a.nome as animal, to_char(o.datacadastro, 'dd/MM/yyyy') "
                    + "as data, o.total, o.veterinario, o.funcionario as atendente, o.situacao "
                    ...
                    + "and p.nome like '"+getCliente()+"%' order by o.codigo desc "
                    + "limit 100";
            ...
            System.out.println("Erro ao ConsultarOrcamentoDAO "+e.getMessage());
            return rs;
        }
    }
    
    public ResultSet FecharOrcamentoDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "select distinct o.codigo, p.nome, o.cliente, o.animal, a.telefone, "
    ...
    public ResultSet ConsultarMaiorCodigoOrcamentoDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "select max(codigo) from orcamento";
            pst = ConectaBanco.con.prepareCall(comando, ResultSet.TYPE_SCROLL_INSENSITIVE, 
                    ResultSet.CONCUR_READ_ONLY);
            rs = pst.executeQuery();
            return rs;
        } catch (Exception e) {
            System.out.println("Erro ao ConsultarMaiorCodigoOrcamentoDAO "+e.getMessage());
            return rs;
        }
    }
}
