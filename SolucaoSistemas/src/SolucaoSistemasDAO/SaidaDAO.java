...
public class SaidaDAO {
    ...
    public boolean CadastrarSaidaDAO(){
        try {
            ConectaBanco.ConectaPostgres();           
            comando = "insert into saida (planocontas, valor, tipo, formapgto, "
                    + "caixa, observacao, datacadastro) "
                    + "values((select codigo from planocontas where descricao = '"+getPlanoContas()+"' "
                    + "and tipo in('1. ATIVO')),?,?,(select codigo from planocontas where descricao = '"+getFormaPgto()+"'), "
                    + "(select max(codigo) from caixa),?,now())";
           ...
           // pst.setString(4, getFormaPgto());
            ...
            System.out.println("Erro ao CadastrarSaidaDAO "+e.getMessage());
            return false;
        }
    }
    
    public boolean AlterarSaidaDAO(){
        try {
            ConectaBanco.ConectaPostgres();
            comando = "update saida set valor = ?, formapgto = (select codigo from planocontas "
                    + "where descricao = '"+getFormaPgto()+"'), observacao = ? "
            ...
            System.out.println("Erro ao AlterarSaidaDAO "+e.getMessage());
            return false;
        }
    }
}
